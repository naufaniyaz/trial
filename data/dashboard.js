...
function renderCard(row) {
  ...
  const hasData = row.months.some((month) => chartValue(month) !== null);
  return `
...
function renderChart(row) {
  ...
  const chartMonths = row.months.filter((month) => chartValue(month) !== null);
  if (!chartMonths.length) return;
  const eventValues = chartMonths.map((month) => chartValue(month));
  const alertData = thresholdSeries(chartMonths, row.alert, "alert");
  const targetData = thresholdSeries(chartMonths, row.target, "target");
  ...
  const chart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Events",
          data: chartMonths.map((month) => chartValue(month)),
          borderColor: "#D2926A",
        ...
        thresholdDataset("Alert", alertData, "#e45454", [4, 3]),
        thresholdDataset("Target", targetData, "#28b85d", [6, 4])
      ].filter(Boolean)
    },
    options: {
      animation: false,
...
function chartValue(month) {
  if (month.events !== null && month.events !== undefined) return month.events;
  return month.value !== null && month.value !== undefined ? month.value : null;
}

function thresholdSeries(months, fallbackValue, key) {
  const fallback = numberOrNull(fallbackValue);
  return months.map((month) => {
    const value = numberOrNull(month[key]);
    return value !== null ? value : fallback;
  });
}

function thresholdDataset(label, data, color, dash) {
  if (!data || data.every((value) => value === null || value === undefined || Number.isNaN(Number(value)))) return null;
  return {
    label,
    data,
    borderColor: color,
